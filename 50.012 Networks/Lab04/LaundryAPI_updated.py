        ...
        if id!=-1:
            if state!=-1:
                if request.method == 'PUT':
                    ...
                    print state
                    quer="Drier "+str(blk)+"-"+str(id)+" is now set to "+newstate
                    print quer
                    print request.headers['Content-Type']=='text/plain'
                    print request.headers['Content-Type']=='application/json'
                    if request.headers['Content-Type']=='text/plain':
                        return quer
                    elif request.headers['Content-Type']=='application/json':
                        return json.dumps("tasfdasdfdf")
                    else:
                        return quer
            else:
...
# Exception Handling for GET requests without arguments
    ...
    if blk==55 or blk==57 or blk==59: 
        ...
        if id!=-1:
            if state!=-1:
                if request.method == 'PUT':
                    washer[blk][id].changeStateUse(state)
                    quer="Washer "+str(blk)+"-"+str(id)+" is now set to "+str(state)
                    if request.headers['Content-Type']=='text/plain':
                        return quer
                    elif request.headers['Content-Type']=='application/json':
                        return json.dumps(quer)  
